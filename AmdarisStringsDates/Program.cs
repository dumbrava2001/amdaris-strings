...
var sentenceCount = text.Split('.').Count(sentence => sentence.Trim().Length > 0);
Console.WriteLine($"\nSentence count: {sentenceCount}");

//- Display how often the word "encapsulation" appears in this string
const string wordToSearch = "encapsulation";
var wordAppearance = text.Split(' ').Count(word => word.Trim().Equals(wordToSearch));
Console.WriteLine($"\nWord {wordToSearch} appears: {wordAppearance}");

//- Display this string in reverse, without using any C# language feature. (Create your own algorithm)
...
Console.WriteLine("\nDisplaying only text with only reversed words. Punctuation and sentence order are the same");

//Determine words count to know what length the words array will have
var wordCount = 0;
...
var formattedString = text.Replace(stringToReplace, replacer);
